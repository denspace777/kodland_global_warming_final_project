# kodland_global_warming_final_project

![глобальное потепление](https://www.google.com/imgres?q=%D0%BA%D0%B0%D1%80%D1%82%D0%B8%D0%BD%D0%BA%D0%B8%20%D0%B3%D0%BB%D0%BE%D0%B1%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D0%BC%20%D0%BF%D0%BE%D1%82%D0%B5%D0%BF%D0%BB%D0%B5%D0%BD%D0%B8%D0%B8&imgurl=https%3A%2F%2Fzooblog.ru%2Fwp-content%2Fuploads%2F2021%2F02%2Fizobrazhenie_2021-02-13_210116.png&imgrefurl=https%3A%2F%2Fzooblog.ru%2Finteresnye-fakty-o-globalnom-poteplenii%2F&docid=BNByN4yv0rUp_M&tbnid=piY9Go5pupOuxM&vet=12ahUKEwiplfeOq8iWAxVy3QIHHTbUAB8QnPAOegUI0AIQAA..i&w=950&h=559&hcb=2&itg=1&ved=2ahUKEwiplfeOq8iWAxVy3QIHHTbUAB8QnPAOegUI0AIQAA)


## мои идеи для проекта:
> 1. порядок представления ситуации для человека:
> введение в курс дела и небольшие факты, основная информация, опросы или задания или советы о экологии, ежедневные новости ситуации потепления, подкрепляем всё историями.

> 2. что должно быть в проекте:
> "мини обучение" помогающие понять где что в тг боте, всё выше перечисленное, красивый и понятный интерфейс (это везде надо), проект должен быть доступным, можно сделать опен сорс


## тип проекта:
> ТГ бот


## библиотеки:
> 1. основные: random, pyTelegramBotAPI
> 2. могут пригодиться: googletrans==4.0.0-rc1 (переводчик), request
> 3. скорей не пригодятся: pyttsx3, speech_recognition, sounddevice


## референс:
> 1. основа от предыдущих проектов - ТГ ботов, может затрону другие свои пректы
> 2. всё остальное пишу с чистого листа


## полезная информация:
> 1. [новости](https://wmo.int/ru)
> 2. [состояние земли](https://science.nasa.gov/climate-change/)
> 3. [изменение климата по России](http://www.igce.ru/climatechange/)
> 4. [о мифах](https://skepticalscience.com/)
